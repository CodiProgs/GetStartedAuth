...
function ErrorHandler({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {children}
      {show && (
        <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} className='fixed bg-themeColorBgSoft top-[30px] left-0 right-0 mx-auto max-w-[300px] w-full px-[30px] py-[20px] h-max min-h-[100px] border border-blueColor z-50 flex items-center flex-col gap-4 rounded-lg'>
